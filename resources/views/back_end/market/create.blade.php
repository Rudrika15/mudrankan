...
<div class="container mt-3">
  ...
  @if ($message = Session::get('success'))
    ...
    <div class="d-flex flex-row-reverse">
      <a href="{{route('market.show')}}" class="btn btn-primary">Back </a>    
    </div>
@if ($errors->any())
...
@endif
  <form action="{{url('backend/market/code')}}" method="post">
    ...
    <div class="mb-3">
      ...
      <textarea class="form-control" id="information" placeholder="{{__('Enter info here')}}" name="information"></textarea>
    </div>
    <div class="mb-3 mt-3">
      <label for="admin_commision">{{__('Admin Commision')}}:</label>
      <input type="text" class="form-control" id="admin_commision" placeholder="{{__('Enter admin commision')}}" name="admin_commision">
    </div>
    <div class="mb-3 mt-3">
      <label for="delivery_fee">{{__('Delivery Fee')}}:</label>
      <input type="text" class="form-control" id="delivery_fee" placeholder="{{__('Enter delivery fee')}}" name="delivery_fee">
    </div>
    <div class="mb-3 mt-3">
      <label for="delivery_range">{{__('Delivery Range')}}:</label>
      <input type="text" class="form-control" id="delivery_range" placeholder="{{__('Enter delivery range')}}" name="delivery_range">
    </div>
    <div class="mb-3 mt-3">
      <label for="default_tax">{{__('Default Tax')}}:</label>
      <input type="text" class="form-control" id="default_tax" placeholder="{{__('Enter default tax')}}" name="default_tax">
    </div>
    <div class="mb-3 mt-3">
